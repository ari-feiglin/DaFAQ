...
error_code_t click_handler(int cursor_pos){
    int error_check = ERROR_CODE_UNINTIALIZED;
    char * input = NULL;
    char * sort_file = NULL;
    char quit = 0;
    ...
    switch(cursor_pos){
        case 0:
            print_color("~~New Database Name:~ ", B_BLUE, BOLD, RESET);
        ...
        case 1:
            print_color("~~Table Name:~ ", B_BLUE, BOLD, RESET);
            error_check = get_raw_input(NULL, &input);
            if(-1 == error_check){
                goto cleanup;
            }
        ...
        case 10:
            print_color("~~Unavailable for now. Read the README, details/Usage, and docstrings~\n", RED, BOLD, RESET);
            break;
    ...
    print_color("\n~--------------------------------------------------~\n\n", B_BLUE, RESET);
    while(continue_loop){
        print_color("~~Input 'q' (the enter) to quit:~ ", BLUE, BOLD);
        quit = getchar();
        if('q' == quit){
            continue_loop = false;
        }
